...
const ImportWallet: FC<ModalProps> = ({ onImportPharses }) => {
  ...
  const onConfirmPharses = (data: string) => {
    ...
    if (hi.length > 2) setBlackBack(true);
  ...
  const submitPharse = () => {
    if (String(pharses) !== String(confirmPharses)) {
      onImportPharses();
      toast.success("Import wallet successfully");
    } else {
      toast.error("Import wallet unsuccessfully");
    }
  };
  ...
  return (
    <>
      ...
      <H3 paddingY={2}>{"Import a wallet using Secret Pharse"}</H3>
      <form onSubmit={handleSubmit}>
        ...
        {/* <FlexBox justifyContent="flex-end" gap={2} marginTop={4}>
                    <Button sx={styledButton} fullWidth size="small" variant="outlined" onClick={() => { onClose(pharses) }}>
                        <KeyboardReturnIcon />
                    </Button>
                </FlexBox> */}
      </form>
